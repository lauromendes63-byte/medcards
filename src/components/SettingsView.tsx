...
import { CURRENT_APP_VERSION, CURRENT_BUILD_LABEL, checkServerVersion, forceAppUpdate } from '../utils/versionManager';
...
export const SettingsView: React.FC<SettingsViewProps> = ({
  ...
  const handleChecarAtualizacao = async () => {
    setIsCheckingUpdate(true);
    setStatusVerificacao('Consultando Vercel na nuvem...');
    try {
      const res = await checkServerVersion();
      if (res.hasUpdate && res.serverInfo) {
        setStatusVerificacao(`Nova versão disponível (${res.serverInfo.buildDateFormatted})! Toque em "Forçar Atualização" abaixo para carregar.`);
      } else {
        setStatusVerificacao(`Você já está na versão mais recente (${CURRENT_BUILD_LABEL})!`);
      }
    } catch {
      setStatusVerificacao('Não foi possível verificar no momento. Verifique sua conexão.');
    } finally {
  ...
  const handleForcarAtualizacao = async () => {
    ...
    setStatusVerificacao('Limpando caches do dispositivo e recarregando do Vercel...');
  ...
  return (
    <div className="w-full space-y-3 pb-6">
      ...
      {/* Bloco 4: Versão e Atualização Contínua Vercel */}
      <div className="bg-white rounded-2xl p-4 border border-slate-200/80 shadow-2xs space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400">
            Versão & Sincronização Vercel
          </h3>
          <span className="px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200/70 text-[10px] font-bold flex items-center gap-1">
        ...
        <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 space-y-2 text-xs">
          <div className="flex items-center justify-between">
            <span className="text-slate-500 text-[11px]">Versão Atual:</span>
            <span className="font-bold text-slate-800">v{CURRENT_APP_VERSION}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-slate-500 text-[11px]">Última Compilação:</span>
            <span className="font-semibold text-slate-700">{CURRENT_BUILD_LABEL}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-slate-500 text-[11px]">Pipeline de Deploy:</span>
        ...
        {statusVerificacao && (
          <div className="p-2.5 rounded-xl bg-blue-50/80 border border-blue-100 text-blue-900 text-xs flex items-start gap-2 animate-in fade-in">
            <Info className="w-4 h-4 text-blue-600 shrink-0 mt-0.5" />
            <span className="text-[11px] leading-relaxed">{statusVerificacao}</span>
          </div>
        )}
        ...
        <div className="flex flex-col sm:flex-row gap-2 pt-1">
          <button
            type="button"
            onClick={handleChecarAtualizacao}
            disabled={isCheckingUpdate || isForcingUpdate}
            className="flex-1 py-2 px-3 rounded-xl bg-slate-100 hover:bg-slate-200 active:scale-95 text-slate-700 text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50"
          >
          ...
          <button
            ...
            className="flex-1 py-2 px-3 rounded-xl bg-blue-600 hover:bg-blue-700 active:scale-95 text-white text-xs font-bold transition-all shadow-xs flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50"
          >
            <RefreshCw className={`w-3.5 h-3.5 ${isForcingUpdate ? 'animate-spin' : ''}`} />
            <span>{isForcingUpdate ? 'Recarregando...' : 'Forçar Atualização'}</span>
          </button>
        </div>

        <p className="text-[10px] text-slate-400 leading-tight">
          O botão &quot;Forçar Atualização&quot; limpa todos os caches locais e garante que o seu celular ou notebook baixe na hora a versão mais nova publicada na nuvem.
        </p>
      </div>
